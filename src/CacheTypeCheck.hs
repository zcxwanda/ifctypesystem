...
typeExp lat env e = case e of
    ...
    BinOp op a b     -> let (validA, levelA) = typeExp lat env a
...
typeLogic lat env e = case e of
    Pred (a :==: b) -> let (validA, levelA) = typeExp lat env a
                           (validB, levelB) = typeExp lat env b
                       in (validA && validB, join lat levelA levelB)
    Pred (a :>=: b) -> let (validA, levelA) = typeExp lat env a
                           (validB, levelB) = typeExp lat env b
                       in (validA && validB, join lat levelA levelB)
    And ls          -> let results = map (typeLogic lat env) ls
...
typeStmt lat env s = case s of
    ...
    If l st sf           -> let (validL, levelL) = typeLogic lat env l
                            in validL && (typeStmt lat env st) && (typeStmt lat env sf)
    While l st           -> let (validL, levelL) = typeLogic lat env l
                            in validL && typeStmt lat env st
    Assign v e           -> let (validE, levelE) = typeExp lat env e
                                varLevel = Just(get env v)
                            in validE && (case varLevel of
                                      Just vLevel -> lte lat levelE vLevel
                                      Nothing -> False)
    ArrAsn arr index e   -> let (validIndex, levelIndex) = typeExp lat env index
